(define (domain domain-5n-6s-5v)
	...
	(:types 
			...
			link-upf0-amf1-ns5 - virtual-link
			link-amf1-smf1-ns5 - virtual-link

			; Network Slice 6 (NS) urllc
			amf0-ns6 - vnf
			smf0-ns6 - vnf
			upf0-ns6 - vnf
			amf1-ns6 - vnf
			...
			link-upf0-amf1-ns6 - virtual-link
			link-amf1-smf1-ns6 - virtual-link
	)
	...
	(:action migrate-upf0-ns4
		...
		:precondition(and
					  ...
					  (link-mapped-to ?link-to-migrate ?from-path)
					  (not(= ?from-path ?to-path))
					  ; enlace a migrar quede en el camino que conecta al nuevo nodo de amf-ns5 y el nodo donde está mapeada
					  ; smf-ns5 
					  (next-vnf ?vnf-to-migrate ?next-vnf)
					  ; nodo donde está mapeada smf-ns5
					  (vnf-mapped-to ?next-vnf ?smf0-ns5-node)
					  ; to-path -> camino entre nodo donde está mapeada smf-ns5 y to-node
		...
		:precondition(and
					  ...
					  (not (= ?from-path2 ?to-path2))
					  (next-vnf ?vnf-to-migrate ?next-vnf)
					  ; nodo donde está mapeada upf-ns5
					  (vnf-mapped-to ?next-vnf ?upf0-ns5-node)
					  ; to-path -> camino entre nodo donde está mapeada upf-ns5 y to-node
	...
	(:action migrate-upf0-ns5
		...
		:parameters (?vnf-to-migrate - upf0-ns5
					 ...
					 ; next vnf on SFC
					 ?next-vnf - amf1-ns5
					 ?amf1-ns5-node - host-node
					 ?link2-to-migrate - link-upf0-amf1-ns5
					 ?from-path2 ?to-path2 - path
					 )
		...
		:precondition(and
					  ...
					  (not(= ?from-path2 ?to-path2))
					  (next-vnf ?vnf-to-migrate ?next-vnf)
					  (vnf-mapped-to ?next-vnf ?amf1-ns5-node)
					  (adjacent-nodes-i-j ?to-path2 ?amf1-ns5-node ?to-node)
					  (adjacent-nodes-j-i ?to-path2 ?to-node ?amf1-ns5-node)
					  (>= (/(embedding-bandwidth ?to-path2)(required-bandwidth-urllc)) 1)
		...
		(:action migrate-amf1-ns5

		:parameters (?vnf-to-migrate - amf1-ns5
					 ?from-node ?to-node - host-node
					 ; prev vnf en la sfc
					 ?prev-vnf - upf0-ns5					 				 
					 ?upf0-ns5-node - host-node
					 ?link1-to-migrate - link-upf0-amf1-ns5
					 ?from-path1 ?to-path1 - path
					 ; next vnf on SFC
					 ?next-vnf - smf1-ns5
					 ?smf1-ns5-node - host-node
					 ?link2-to-migrate - link-amf1-smf1-ns5
					 ?from-path2 ?to-path2 - path
					 )
		...
		:precondition(and
					  ...
					  (not(= ?from-path1 ?to-path1))
					  (next-vnf ?prev-vnf ?vnf-to-migrate)
					  (vnf-mapped-to ?prev-vnf ?upf0-ns5-node)
					  (adjacent-nodes-i-j ?to-path1 ?upf0-ns5-node ?to-node)
					  (adjacent-nodes-j-i ?to-path1 ?to-node ?upf0-ns5-node)
					  (>= (/(embedding-bandwidth ?to-path1)(required-bandwidth-urllc)) 1)
					  ...
					  (not(= ?from-path2 ?to-path2))
					  (next-vnf ?vnf-to-migrate ?next-vnf)
					  (vnf-mapped-to ?next-vnf ?smf1-ns5-node)
					  (adjacent-nodes-i-j ?to-path2 ?smf1-ns5-node ?to-node)
					  (adjacent-nodes-j-i ?to-path2 ?to-node ?smf1-ns5-node)
					  (>= (/(embedding-bandwidth ?to-path2)(required-bandwidth-urllc)) 1)
		...
		(:action migrate-smf1-ns5

		:parameters (?vnf-to-migrate - smf1-ns5
					 ?from-node ?to-node - host-node
					 ; prev vnf on SFC
					 ?prev-vnf - amf1-ns5					 
					 ?amf1-ns5-node - host-node
					 ?link-to-migrate - link-amf1-smf1-ns5
					 ?from-path ?to-path - path
					 )
		...
		:precondition(and
					  ...
					  (next-vnf ?prev-vnf ?vnf-to-migrate)
					  ; nodo donde está mapeada smf-ns5
					  (vnf-mapped-to ?prev-vnf ?amf1-ns5-node)
					  ; to-path -> camino entre nodo donde está mapeada smf-ns5 y to-node
					  (adjacent-nodes-i-j ?to-path ?amf1-ns5-node ?to-node)
					  (adjacent-nodes-j-i ?to-path ?to-node ?amf1-ns5-node)					 
					  (>= (/(embedding-bandwidth ?to-path)(required-bandwidth-urllc)) 1)
					 )
		...
		:precondition(and
					  ...
					  (link-mapped-to ?link-to-migrate ?from-path)
					  (not(= ?from-path ?to-path))
					  ; enlace a migrar quede en el camino que conecta al nuevo nodo de amf-ns6 y el nodo donde está mapeada
					  ; smf-ns6 
					  (next-vnf ?vnf-to-migrate ?next-vnf)
					  ; nodo donde está mapeada smf-ns6
					  (vnf-mapped-to ?next-vnf ?smf0-ns6-node)
					  ; to-path -> camino entre nodo donde está mapeada smf-ns6 y to-node
		...
		:precondition(and
					  ...
					  (not (= ?from-path2 ?to-path2))
					  (next-vnf ?vnf-to-migrate ?next-vnf)
					  ; nodo donde está mapeada upf-ns6
					  (vnf-mapped-to ?next-vnf ?upf0-ns6-node)
					  ; to-path -> camino entre nodo donde está mapeada upf-ns6 y to-node
	...
	(:action migrate-upf0-ns6
		...
		:parameters (?vnf-to-migrate - upf0-ns6
					 ...
					 ; next vnf on SFC
					 ?next-vnf - amf1-ns6
					 ?amf1-ns6-node - host-node
					 ?link2-to-migrate - link-upf0-amf1-ns6
					 ?from-path2 ?to-path2 - path
					 )
		...
		:precondition(and
					  ...
					  (not(= ?from-path2 ?to-path2))
					  (next-vnf ?vnf-to-migrate ?next-vnf)
					  (vnf-mapped-to ?next-vnf ?amf1-ns6-node)
					  (adjacent-nodes-i-j ?to-path2 ?amf1-ns6-node ?to-node)
					  (adjacent-nodes-j-i ?to-path2 ?to-node ?amf1-ns6-node)
					  (>= (/(embedding-bandwidth ?to-path2)(required-bandwidth-urllc)) 1)
		...
		(:action migrate-amf1-ns6

		:parameters (?vnf-to-migrate - amf1-ns6
					 ?from-node ?to-node - host-node
					 ; prev vnf en la sfc
					 ?prev-vnf - upf0-ns6					 				 
					 ?upf0-ns6-node - host-node
					 ?link1-to-migrate - link-upf0-amf1-ns6
					 ?from-path1 ?to-path1 - path
					 ; next vnf on SFC
					 ?next-vnf - smf1-ns6
					 ?smf1-ns6-node - host-node
					 ?link2-to-migrate - link-amf1-smf1-ns6
					 ?from-path2 ?to-path2 - path
					 )
		...
		:precondition(and
					  ...
					  (not(= ?from-path1 ?to-path1))
					  (next-vnf ?prev-vnf ?vnf-to-migrate)
					  (vnf-mapped-to ?prev-vnf ?upf0-ns6-node)
					  (adjacent-nodes-i-j ?to-path1 ?upf0-ns6-node ?to-node)
					  (adjacent-nodes-j-i ?to-path1 ?to-node ?upf0-ns6-node)
					  (>= (/(embedding-bandwidth ?to-path1)(required-bandwidth-urllc)) 1)
					  ...
					  (not(= ?from-path2 ?to-path2))
					  (next-vnf ?vnf-to-migrate ?next-vnf)
					  (vnf-mapped-to ?next-vnf ?smf1-ns6-node)
					  (adjacent-nodes-i-j ?to-path2 ?smf1-ns6-node ?to-node)
					  (adjacent-nodes-j-i ?to-path2 ?to-node ?smf1-ns6-node)
					  (>= (/(embedding-bandwidth ?to-path2)(required-bandwidth-urllc)) 1)
		...
		(:action migrate-smf1-ns6

		:parameters (?vnf-to-migrate - smf1-ns6
					 ?from-node ?to-node - host-node
					 ; prev vnf on SFC
					 ?prev-vnf - amf1-ns6					 
					 ?amf1-ns6-node - host-node
					 ?link-to-migrate - link-amf1-smf1-ns6
					 ?from-path ?to-path - path
					 )
		...
		:precondition(and
					  ...
					  (next-vnf ?prev-vnf ?vnf-to-migrate)
					  ; nodo donde está mapeada smf-ns6
					  (vnf-mapped-to ?prev-vnf ?amf1-ns6-node)
					  ; to-path -> camino entre nodo donde está mapeada smf-ns6 y to-node
					  (adjacent-nodes-i-j ?to-path ?amf1-ns6-node ?to-node)
					  (adjacent-nodes-j-i ?to-path ?to-node ?amf1-ns6-node)					 
					  (>= (/(embedding-bandwidth ?to-path)(required-bandwidth-urllc)) 1)
					 )
